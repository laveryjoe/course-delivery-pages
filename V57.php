...
function format_transcript($transcript_text) {
    ...
    foreach ($lines as $line) {
        ...
        $formatted .= sprintf(
            '<li class="transcript-item" data-time="%s">
                <span class="transcript-timestamp">%s</span>
                <span class="transcript-text">%s</span>
            </li>',
            $timestamp, $timestamp, htmlspecialchars($line)
        );
...
<head>
    ...
    <script>
        (function() {
            try {
                const theme = localStorage.getItem('theme') || '<?php echo DEFAULT_THEME_MODE; ?>';
                if (theme === 'dark') {
    ...
    <style>
        ...
        /* Dark Theme Classes - Applied via JavaScript */
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        [data-theme="dark"] .mobile-theme-toggle .mobile-theme-icon::before {
            content: "☀️ ";
        }

        [data-theme="dark"] .mobile-theme-toggle::after {
            content: " (Light Mode)";
        }
        ...
        .main { max-width: 90%; width: 100%; margin: 0 auto; padding: 2rem; display: grid; grid-template-columns: 1fr 380px; gap: 4rem; align-items: start; }
        ...
        .learning-objectives h4 { color: var(--blue-primary); margin-bottom: 0.5rem; font-size: 0.9rem; font-weight: 600; }
        .learning-objectives ul { margin: 0; padding-left: 1.25rem; }
        .learning-objectives li { color: var(--text-primary); margin: 0.25rem 0; font-size: 0.9rem; }

        @media (min-width: 1400px) {
            .header-container { max-width: 85%; }
            .main { max-width: 85%; grid-template-columns: 1fr 420px; }
        }

        @media (min-width: 1600px) {
            .header-container { max-width: 80%; }
            .main { max-width: 80%; grid-template-columns: 1fr 450px; }
        }

        @media (max-width: 768px) {
            .nav { display: none; }
            .mobile-menu { display: block; }
            .mobile-lessons-toggle { display: flex; }
            .main { grid-template-columns: 1fr; gap: 2rem; padding: 1rem 0.5rem; max-width: 95%; }
            .content { padding: 1rem; }
            .sidebar { display: none; }
            .next-video { flex-direction: column; text-align: center; }
            .next-video-thumb { width: 100%; height: 120px; }
    ...
    <style>
        ...
        .module-header:hover { background: var(--bg-quaternary); border-color: var(--border-tertiary); }
        .module-header.active { background: var(--blue-secondary); border-color: var(--blue-primary); color: var(--text-secondary); }
        .module-title { flex: 1; display: flex; align-items: center; gap: 0.75rem; }
        .module-icon { width: 20px; height: 20px; background: var(--blue-primary); border-radius: 4px; display: flex; align-items: center; justify-content: center; color: white; font-size: 10px; font-weight: 600; }
        .module-arrow { transition: transform 0.2s; font-size: 0.8rem; color: var(--text-tertiary); }
        .module-header.active .module-arrow { transform: rotate(90deg); }
        .module-lessons { max-height: 0; overflow: hidden; transition: max-height 0.3s ease-out; background: var(--bg-tertiary); border-radius: 6px; margin-bottom: 1rem; }
        .module-lessons.expanded { max-height: 1000px; transition: max-height 0.3s ease-in; }
        .episode-list { list-style: none; padding: 0.5rem; }
        .episode-item { margin-bottom: 0.3rem; }

        /* Mobile Two-Level Dropdown */
        .mobile-module-item { display: flex; align-items: center; justify-content: space-between; padding: 1rem 2rem; color: var(--text-primary); font-weight: 600; border-bottom: 1px solid var(--bg-tertiary); background: var(--bg-tertiary); font-size: 0.9rem; cursor: pointer; transition: all 0.2s; }
        ...
        .mobile-lesson-item:hover { background: var(--bg-tertiary); color: var(--blue-primary); }
    ...
    <style>
        ...
        .mobile-lesson-item:hover { background: #f9fafb; color: #0046dd; }
    ...
    <style>
        ...
        .play-icon { width: 16px; height: 16px; background: var(--blue-primary) !important; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white !important; font-size: 10px; flex-shrink: 0; }
        .episode-link.current .play-icon { background: #ffffff !important; color: var(--blue-primary) !important; }
...
<body>
	<header class="header">
		...
		<nav class="mobile-nav" id="mobile-nav">
			...
			<button class="mobile-theme-toggle" id="mobile-theme-toggle">
				<span class="mobile-theme-icon">🌙</span> Dark Mode
			</button>
		</nav>
					...
					foreach ($modules as $module):
						...
						if (!empty($module_lessons)): ?>
							<div class="mobile-module-item" data-module-id="<?php echo $module['id']; ?>">
								...
								<span class="mobile-module-arrow">▶</span>
							</div>
    ...
    <main class="main">
                ...
                <div class="tab-content" id="transcript">
                    <div style="margin-bottom: 1.5rem;">
                        <div style="display: flex; gap: 1rem; margin-bottom: 1rem; flex-wrap: wrap;">
                            <button class="transcript-control active" data-speed="1x" style="padding: 0.5rem 1rem; border: 1px solid #d1d5db; background: #0046dd; color: white; border-radius: 6px; font-size: 0.9rem; cursor: pointer; font-weight: 500;">1x Speed</button>
                            <button class="transcript-control" data-speed="1.25x" style="padding: 0.5rem 1rem; border: 1px solid #d1d5db; background: white; color: #4b5563; border-radius: 6px; font-size: 0.9rem; cursor: pointer;">1.25x</button>
                            <button class="transcript-control" data-speed="1.5x" style="padding: 0.5rem 1rem; border: 1px solid #d1d5db; background: white; color: #4b5563; border-radius: 6px; font-size: 0.9rem; cursor: pointer;">1.5x</button>
                            <button class="transcript-search" style="padding: 0.5rem 1rem; border: 1px solid #d1d5db; background: white; color: #4b5563; border-radius: 6px; font-size: 0.9rem; cursor: pointer;">🔍 Search Transcript</button>
                        </div>
                    </div>
            ...
            <div class="up-next">
                ...
                <div class="next-video" id="next-lesson">
                    ...
                    <div class="next-video-content">
                        <h3>Launch Fundamentals <span style="color: #6b7280; font-weight: 400; font-size: 0.9rem;">• 10 min</span></h3>
        ...
        <aside class="sidebar">
            ...
            <?php if (ADVANCED_COURSE_MODE): ?>
                ...
                ?>
                    <div class="episode-section">
                        <div class="module-header <?php echo $is_current_module ? 'active' : ''; ?>" data-module-id="<?php echo $module['id']; ?>">
                            <div class="module-title">
                                <div class="module-icon"><?php echo $module['order_index'] ?? 'M'; ?></div>
                                <span><?php echo htmlspecialchars($module['title'] ?? 'Module ' . $module['id']); ?></span>
                            </div>
            ...
            <div style="margin-top: 2rem; padding: 1rem; background: #eff6ff; border-radius: 8px; border: 1px solid #bfdbfe;">
                <h4 style="font-size: 0.9rem; font-weight: 600; color: #0046dd; margin-bottom: 0.5rem;">📊 Course Progress</h4>
    ...
    <script>
        ...
        function initModuleAccordion() {
            ...
            moduleHeaders.forEach(function(header) {
                header.addEventListener('click', function() {
                    ...
                    // Close all modules first (accordion behavior)
                    moduleHeaders.forEach(function(h) { h.classList.remove('active'); });
                    document.querySelectorAll('.module-lessons').forEach(function(ml) { ml.classList.remove('expanded'); });
                    ...
                    if (transcriptEl && lessonData.transcript_text) {
                        ...
                        lines.forEach(function(line) {
                            if (line.trim()) {
                                ...
                                transcriptHTML += '<li class="transcript-item">';
        ...
        function getStoredTheme() {
            try {
                return localStorage.getItem('theme') || '<?php echo DEFAULT_THEME_MODE; ?>';
            } catch (error) {
                return '<?php echo DEFAULT_THEME_MODE; ?>';
            }
        }
